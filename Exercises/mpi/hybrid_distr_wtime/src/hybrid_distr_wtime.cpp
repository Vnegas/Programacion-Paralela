    ...
    if (argc == 3) {  // Check if thread count was specified
      ...
      if (process_number == 0) {
        ...
        for (int target = 1; target < process_count; ++target) {
          if (MPI_Send(&overall_start, 1, MPI_INT, target, 0
              , MPI_COMM_WORLD) != MPI_SUCCESS) {
            fail("could not send overall start");
          }
          if (MPI_Send(&overall_finish, 1, MPI_INT, target, 0
              , MPI_COMM_WORLD) != MPI_SUCCESS) {
            fail("could not send overall finish");
          }
        }
      } else {
        if (MPI_Recv(&overall_start, 1, MPI_INT, 0, 0, MPI_COMM_WORLD
            , MPI_STATUS_IGNORE) != MPI_SUCCESS ) {
          fail("could not receive overall start");
        }
        if (MPI_Recv(&overall_finish, 1, MPI_INT, 0, 0, MPI_COMM_WORLD
            , MPI_STATUS_IGNORE) != MPI_SUCCESS ) {
          fail("could not receive overall finish");
        }
      }
    }
