/// @author Sebastián Venegas Brenes
/// @copyright 2023 Sebastián Venegas Brenes
...
int main(int argc, char* argv[]) {
  ...
  if ( shared_data == NULL )
    return (void)fprintf(stderr, "could not allocate shared memory\n"), 1;
  ...
  destroy_terrain(shared_data->terrain, shared_data->rows);
  // free(shared_data->terrain);
...
char** create_terrain(const size_t rows, const size_t columns) {
  ...
  for (size_t row = 0; row < rows; ++row)
    if ((terrain[row] = calloc(columns, sizeof(char))) == NULL)
      return destroy_terrain(terrain, row), NULL;
  return terrain;
}
...
int create_threads(shared_data_t* shared_data) {
  pthread_t* threads = (pthread_t*) malloc(shared_data->thread_count
    * sizeof(pthread_t));
  if (threads == NULL)
    return (void)fprintf(stderr, "error: could not allocate memory for "
      "%zu threads\n", shared_data->thread_count), 2;

  private_data_t* private_data = (private_data_t*)
    calloc(shared_data->thread_count, sizeof(private_data_t));
  if (private_data == NULL)
    return (void)fprintf(stderr, "error: could not allocate private memory for "
...
void find_maximum_perimeter(const size_t thread_num
  ...
  // Repeat concurrently|in parallel (ciclically) top left row
  // for each row of terrain except last one
  for (size_t top_left_row = thread_num; top_left_row
      < shared_data->rows - 1; top_left_row += shared_data->thread_count) {
    // Repeat top left column for each column of terrain except last one
    for (size_t top_left_column = 0; top_left_column
        < shared_data->columns - 1; ++top_left_column) {
      // Create local perimeter as the result of finding maximum perimeter
      // that can be formed from row and column
      coordinate_t local_bottom_right = find_maximum_local_perimeter
        (top_left_row, top_left_column, shared_data);
...
// Find maximum perimeter from (top left row, top left column):
coordinate_t find_maximum_local_perimeter(const size_t top_left_row
    , const size_t top_left_column, const shared_data_t* shared_data) {
  ...
  for (size_t bottom_right_row = top_left_row + 1; bottom_right_row
      < shared_data->rows; ++bottom_right_row) {
    // Repeat bottom right column from top left column + 1 to the last one
    for (size_t bottom_right_column = top_left_column + 1; bottom_right_column
        < shared_data->columns; ++bottom_right_column) {
      // If can form rectangle from top left cell to bottom right cell
      if (can_form_rectangle(top_left_row, top_left_column, bottom_right_row
        , bottom_right_column, shared_data)) {
        // Create local perimeter as result of calculate perimeter
        size_t perimeter = calculate_perimeter((coordinate_t){top_left_row
          , top_left_column}, (coordinate_t){bottom_right_row
          , bottom_right_column});
        // If local perimeter is larger than the maximum perimeter
...
bool can_form_rectangle(const size_t top_left_row, const size_t top_left_column
  ...
  for (size_t column = top_left_column; column <= bottom_right_column
      ; ++column) {
...
void print_maximum_perimeter(const shared_data_t* shared_data) {
  printf("%zu (%zu,%zu)-(%zu,%zu)\n", shared_data->maximum_perimeter
    , shared_data->maximum_top_left.row + 1, shared_data
      ->maximum_top_left.column + 1
    , shared_data->maximum_bottom_right.row + 1, shared_data
      ->maximum_bottom_right.column + 1);
}
