...
FactReqAnalizer::FactReqAnalizer() {}

FactReqAnalizer::~FactReqAnalizer() {}

int FactReqAnalizer::run() {
  this->consumeForever();
...
void FactReqAnalizer::consume(reqRes_t data) {
  ...
  std::regex inQuery("^/fact(/|\\?numbers=)([-?\\d+,?]*)$");
...
void FactReqAnalizer::split(std::string str, char del
    , std::vector<int64_t> &numbers) {
  std::string number;
...
void FactReqAnalizer::replaceString(std::string& uri) {
...
void FactReqAnalizer::serveHomepage(reqRes_t data) {
