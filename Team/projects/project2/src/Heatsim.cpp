...
void HeatSim::run(char *argv[]) {
  this->analyze_arguments(argv);
  this->extract_data();
  this->fill_matrix();
  this->write_report();
  this->write_binary_matrix();
}
...
void HeatSim::extract_data() {
  for (size_t i = 0; i < this->info.size(); i++) {
    std::string aux = "";
    std::vector<std::string> tmp;
    for (size_t j = 0; j < this->info[i].size(); j++) {
      if (this->info[i][j] != ' ') {
        aux = aux + this->info[i][j];
        if (j == this->info[i].size() - 1) {
          tmp.push_back(aux);
        }
      } else {
        if (aux != "") {
    ...
    for (size_t k = 0; k < tmp.size(); k++) {
      switch (k) {
        case 0:
          this->jobs[i].plate_name = tmp[k];
          break;
        case 1:
          this->jobs[i].delta_time = std::stod(tmp[k]);
          break;
        case 2:
          this->jobs[i].diffusion = std::stod(tmp[k]);
          break;
        case 3:
          this->jobs[i].dimensions = std::stod(tmp[k]);
          break;
        case 4:
          this->jobs[i].epsilon = std::stod(tmp[k]);
          break;
        default:
          break;
      }
...
void HeatSim::write_report() {
  std::vector<std::string> results;
  for (size_t i = 0; i < this->jobs.size(); i++) {
    std::stringstream buffer;
    buffer
      << this->jobs[i].plate_name << "\t"
      << this->jobs[i].delta_time << "\t"
      << this->jobs[i].diffusion << "\t"
      << this->jobs[i].dimensions << "\t"
      << this->jobs[i].epsilon << "\t"
      << this->jobs[i].k << "\t"
      << format_time(this->jobs[i].time_elapsed) << "\n";
    std::string aux = buffer.str();
    results.push_back(aux);
  }

  std::ofstream file;
  file.open(this->path_no_extension + ".tsv", std::ios::out);
  if (file.is_open()) {
