...
void greet(int process_number, int process_count, const char* process_hostname)
{
  ...
  if (process_number != 0) {
    ...
    std::cout << std::endl << buffer.str();
    for (int current = 1; current < process_count; ++current) {
      std::vector<char> message(MESSAGE_CAP);
      // receive(&message, message_capacity, source)
      if (MPI_Recv(&message[0], /*count*/ MESSAGE_CAP, MPI_CHAR, MPI_ANY_SOURCE
