...
const char* const usage =
  ...
  "  max_queue_capacity   capacity of queue\n"
  "\n"
...
int ProducerConsumerTest::start(int argc, char* argv[]) {
  ...
  this->dispatcher->getConsumingQueue()->setBufferSize(this->maxQueueCapacity);
  ...
  for ( size_t index = 0; index < this->consumerCount; ++index ) {
    ...
    this->consumers[index]->getConsumingQueue()->setBufferSize(this
      ->maxQueueCapacity);
  }
  ...
  std::cout << "Execution time: " << elapsedTime.count() << std::endl;
...
int ProducerConsumerTest::analyzeArguments(int argc, char* argv[]) {
  ...
  if ( argc == 7 ) {
    this->maxQueueCapacity = std::atoi(argv[index++]);
  } else {
    this->maxQueueCapacity = SEM_VALUE_MAX;
  }
