...
int join_threads(size_t count, thread_data_t* thread_data);
...
simulation_t* simulation_create() {
  ...
  if (simulation) {
    ...
    pthread_mutex_init(&simulation->can_access_next_unit, NULL);
...
thread_data_t* create_threads(size_t count, void*(*subroutine)(void*)
    , void* data) {
  pthread_t* threads = (pthread_t*) calloc(count, sizeof(pthread_t));
  thread_data_t* thread_data = (thread_data_t*) 
    calloc(count, sizeof(thread_data_t));
  if (threads && thread_data) {
    for (size_t index = 0; index < count; ++index) {
      thread_data[index].thread_number = index;
      thread_data[index].simulation = (simulation_t *) data;
      thread_data[index].total_threads = count;
      if (pthread_create(&threads[index], /*attr*/ NULL, subroutine
        , &thread_data[index]) == EXIT_SUCCESS) {
      } else {
        fprintf(stderr, "error: could not create thread %zu\n", index);
        join_threads(index, thread_data);
        free (thread_data);
        return NULL;
      }
    }
...
int join_threads(size_t count, thread_data_t* thread_data) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    error += pthread_join(thread_data[index].self, /*value_ptr*/ NULL);
  }
  free(thread_data);
  return error;
}
...
int create_consumers_producers(simulation_t* simulation) {
  ...
  thread_data_t* producers = create_threads(simulation->producer_count, produce
    , simulation);
  thread_data_t* consumers = create_threads(simulation->consumer_count, consume
    , simulation);
